...
export class ResetPasswordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.otpCode = this.route.snapshot.queryParams['otp'] || '';
  ...
  onSubmit(): void {
    ...
    if (this.token) {
      ...
      this.authService.resetPassword(this.email, this.otpCode, this.resetForm.value.password, this.resetForm.value.confirmPassword).subscribe({
        next: () => {
          this.isSubmitted = true;
          this.successMessage = 'Password has been reset successfully.';
          this.isLoading = false;
